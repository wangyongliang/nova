...
void move(int k)
{
	...
	for(i=0;i<3;i++)
	{
		...
		xx=snake[k].x+dx[dir];
		yy=snake[k].y+dy[dir];
		if(xx>=0&&xx<n&&yy>=0&&yy<n&&((a[xx][yy][1]<snake[k].c&&t>=a[xx][yy][0])||(a[xx][yy][1]>=snake[k].c&&t>a[xx][yy][0])))
		{
			cnt++;
			snake[k].x=xx;
			snake[k].y=yy;
...
void make_map(int x,int y)
{
	int i;
	int xx,yy;
	if(a[snake[num].x][snake[num].y][0]-a[x][y][0]>=snake[num].time-1) return ;
	for(i=0;i<4;i++)
	{
		xx=x+dx[i];
		yy=y+dy[i];
		if(xx>=0&&xx<n&&yy>=0&&yy<n&&a[xx][yy][0]==a[x][y][0]-1&&a[xx][yy][1]==a[x][y][1])
		{
			s[xx][yy]=a[x][y][1]-'A'+'a';
			make_map(xx,yy);
			return;
...
int main()
{
	...
	while(scanf("%d%d",&n,&m)!=EOF)
	{
		...
		for(num--;num>=0;num--)
		{
			...
			make_map(snake[num].x,snake[num].y);
		}
