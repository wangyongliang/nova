...
void dfs(int n,int fa,int deep,int id)
{
	...
	for(i=0;i<a[n].size();i++)
	{
		...
		{	
			node tp=a[n][i];
			if(vis[a[n][i].x]==0)//如果之前的没有被访问过。
			{		
				stack[top].x=n;//每一个id=-1的标志代表的是一个分支。
				stack[top].id=-1;
				top++;
				stack[top].x=n;
				...
				if(low[a[n][i].x]>=dfsn[n]) //这个分支下没有后退边
				{
					while(1)
					{
			...
			{
				if(dfsn[a[n][i].x]<dfsn[n])/*本身自己就是一个后退边，注意low[n]=min(low[n],dfsn[a[n][i].x])
										   而不是low[n]=min(low[n],low[a[n][i].x])*/
				{
					stack[top].x=n;
					stack[top].id=a[n][i].id;
					top++;
